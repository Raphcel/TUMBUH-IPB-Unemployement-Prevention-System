    ...
    useEffect(() => {
        ...
        if (user.role === 'student') {
            applicationsApi
                ...
                .catch(() => {});
        } else if (user.role === 'hr' && user.company_id) {
            opportunitiesApi
                .listByCompany(user.company_id)
                .then((data) => {
                            ...
                            <AnimatePresence>
                                {selectedEvents.length > 0 ? (
                                    selectedEvents.map((event) => (
                                        ...
                                        >
                                            <div className={`w-1 rounded-full ${event.type === 'Deadline' ? 'bg-red-400' : event.type === 'Application' ? 'bg-green-400' : 'bg-accent'}`}></div>
                                            <div className="flex-1">
                                                ...
                                                <div className="flex items-center gap-2 mt-0.5">
                                                    ...
                                                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${event.type === 'Deadline' ? 'bg-red-50 text-red-600' : event.type === 'Application' ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'
                                                        }`}>
                                                        {event.type}
                                                    </span>
                                                </div>
